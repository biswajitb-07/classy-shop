// File guide: Jewellery source file.
// This file belongs to the current app architecture and has a focused responsibility within its module/folder.
import { useEffect, useMemo, useState } from "react";
import { useParams } from "react-router-dom";
import { CgCloseR } from "react-icons/cg";
import JewelleryProductCard from "./JewelleryProductCard.jsx";
...
const Jewellery = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetJewelleryItemsQuery();
  const { data: categories } = useGetVendorCategoriesQuery();

  const categoryData = categories?.[0]?.categories || [];
  const allProducts = data?.jewelleryItems ?? [];
  ...
  if (isError) {
    return (
      <ErrorMessage
        title="Failed to load jewellery products"
        message="We couldn’t fetch the jewellery items right now. Please check your internet connection or try again."
        onRetry={refetch}
      />
    );
  }
  ...
  return (
    <div>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-full md:w-[26rem] bg-white shadow-xl z-40 transition duration-400 ease-in-out px-7 pb-20 ${
      ...
      <div className="container grid gap-6">
        <main className="lg:col-span-9 pb-10">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-semibold">
                {normalizeForDisplay(thirdcategory || subcategory || "Jewellery")}
              </h1>
