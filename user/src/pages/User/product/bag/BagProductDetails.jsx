...
const BagProductDetails = () => {
  ...
  const product =
    !isLoading && data?.bagItems?.find((item) => String(item._id) === productId);

  useTrackAiProductClick(product);

  const relatedProducts =
    !isLoading &&
    data?.bagItems?.filter(
      (item) =>
  ...
  useEffect(() => {
    ...
    setCompareActive(isProductCompared(product._id, "Bag"));
  }, [product?._id]);
  ...
  const handleAddToCart = async (redirectToCheckout = false) => {
    ...
    if (redirectToCheckout) {
      const buyNowItems = [buildBuyNowItem({ product, productType: "Bag" })];
  ...
  const handleCompare = () => {
    ...
    const result = toggleCompareProduct(
      buildCompareProduct(product, "Bag", `/bag/bag-product-details/${product._id}`)
    );
        ...
        <div className="flex flex-col xl:flex-row gap-5 md:gap-7">
          ...
          <div className="flex-1 flex flex-col gap-3 sm:gap-4">
            ...
            <p className="mt-2 text-gray-700 text-xs sm:text-sm tracking-wider">
              {product.description ||
                "This bag is designed to blend style, comfort, and daily utility."}
            </p>
            ...
            <div className="mt-4 grid w-full gap-3 sm:max-w-md sm:place-items-end sm:self-end">
              <button
                ...
                disabled={addLoading}
                className="w-full rounded-md border border-red-500 bg-white px-8 py-2 text-xs font-medium text-red-500 transition hover:bg-red-50 disabled:cursor-not-allowed sm:py-3 sm:text-sm"
              >
                {addLoading ? (
                  <AuthButtonLoader />
                ) : (
