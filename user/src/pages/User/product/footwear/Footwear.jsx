// File guide: Footwear source file.
// This file belongs to the current app architecture and has a focused responsibility within its module/folder.
import { useEffect, useMemo, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import FootwearProductCard from "./FootwearProductCard.jsx";
...
const Footwear = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetFootwearItemsQuery();
  const { data: categories } = useGetVendorCategoriesQuery();

  const categoryData = categories?.[0]?.categories || [];

  const allProducts = data?.footwearItems ?? [];
  ...
  if (isError) {
    return (
      <ErrorMessage
        title="Failed to load footwear products"
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-semibold">
                {normalizeForDisplay(thirdcategory || subcategory || "Footwear")}
              </h1>
