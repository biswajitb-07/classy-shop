// File guide: ElectronicsProductCard source file.
...
const ElectronicsProductCard = ({ products = [], isLoading = false }) => {
  ...
  const handleAddToCartFromModal = async (product, { ram, storage }) => {
    if (!isAuthenticated) {
      toast.error("please login");
      return;
    }
  ...
  if (isLoading) return <ElectronicProductCardSkeleton />;
  if (!products)
    return (
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-4 mb-5">
        {products.map((p) => {
          const discount =
            ...
            >
              ...
              <div className="relative group overflow-hidden">
                <img
                  onClick={() =>
                    navigate(
                      `/electronics/electronics-product-details/${p._id}`
                    )
                  }
                  src={p.image[0]}
                  alt={p.name}
                ...
                <div className="absolute top-2 right-2 flex flex-col gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-all">
                  ...
                  <button
                    ...
                    className="bg-white p-1.5 rounded-full shadow hover:scale-110 hover:bg-red-500 hover:text-white transition cursor-pointer"
                  >
              ...
              <div
                onClick={() =>
                  navigate(`/electronics/electronics-product-details/${p._id}`)
                }
              ...
              >
                ...
                <button
                  ...
                  className="mt-2 w-full bg-white border border-red-500 text-red-500 hover:bg-red-500 hover:text-white py-2 md:py-3 rounded-md text-[10px] md:text-[13px] font-medium transition flex items-center justify-center gap-5 disabled:cursor-not-allowed cursor-pointer"
                >
                  {cartLoadingId === p._id ? (
                    <AuthButtonLoader />
                  ) : (
