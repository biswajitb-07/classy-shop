// File guide: Wellness source file.
// This file belongs to the current app architecture and has a focused responsibility within its module/folder.
import { useEffect, useMemo, useState } from "react";
import { useParams } from "react-router-dom";
import { CgCloseR } from "react-icons/cg";
import WellnessProductCard from "./WellnessProductCard.jsx";
...
const Wellness = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetWellnessItemsQuery();
  const { data: categories } = useGetVendorCategoriesQuery();

  const categoryData = categories?.[0]?.categories || [];
  const allProducts = data?.wellnessItems ?? [];
  ...
  if (isError) {
    return (
      <ErrorMessage
        title="Failed to load wellness products"
        message="We couldn’t fetch the wellness items right now. Please check your internet connection or try again."
        onRetry={refetch}
      />
    );
  }
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-semibold">
                {normalizeForDisplay(thirdcategory || subcategory || "Wellness")}
              </h1>
