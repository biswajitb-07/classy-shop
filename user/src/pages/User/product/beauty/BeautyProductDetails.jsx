...
const BeautyProductDetails = () => {
  const { data, isLoading, refetch } = useGetBeautyItemsQuery();
  ...
  const product =
    !isLoading &&
    data?.beautyItems?.find((item) => String(item._id) === productId);

  useTrackAiProductClick(product);

  const relatedProducts =
    !isLoading &&
    data?.beautyItems?.filter(
      (item) =>
  ...
  useEffect(() => {
    ...
    setCompareActive(isProductCompared(product._id, "Beauty"));
  }, [product?._id]);
  ...
  const handleAddToCart = async (redirectToCheckout = false) => {
    ...
    if (redirectToCheckout) {
      const buyNowItems = [buildBuyNowItem({ product, productType: "Beauty" })];
  ...
  const handleCompare = () => {
    ...
    const result = toggleCompareProduct(
      buildCompareProduct(
        product,
        "Beauty",
        `/beauty/beauty-product-details/${product._id}`
      )
    );
        ...
        <div className="flex flex-col xl:flex-row gap-5 md:gap-7">
          ...
          <div className="flex-1 flex flex-col gap-3 sm:gap-4">
            ...
            <p className="mt-2 text-gray-700 text-xs sm:text-sm tracking-wider">
              {product.description ||
                "This beauty product is selected for quality care, confidence, and daily glow."}
            </p>
