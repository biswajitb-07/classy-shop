// File guide: BeautyProductCard source file.
...
const BeautyProductCard = ({ products = [], isLoading = false }) => {
  ...
  const handleShare = async (product, e) => {
    e.stopPropagation();
    const productUrl = `${window.location.origin}/beauty/beauty-product-details/${product._id}`;
  ...
  if (isLoading) return <BeautyProductCardSkeleton />;
  if (!products)
    return (
          ...
          return (
            ...
            >
              <div className="relative group overflow-hidden">
                <img
                  onClick={() => navigate(`/beauty/beauty-product-details/${p._id}`)}
                  src={p.image[0]}
                  alt={p.name}
                ...
                {p.image[1] && (
                  <img
                    onClick={() =>
                      navigate(`/beauty/beauty-product-details/${p._id}`)
                    }
                    src={p.image[1]}
                    alt={p.name}
              ...
              >
                ...
                <p className="text-sm font-semibold text-gray-500">
                  Qty:
                  <span className="text-red-500 px-1">{p.inStock} kg</span>
                </p>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
...
const BeautyProductCardSkeleton = () => (
