...
const Beauty = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetBeautyItemsQuery();
  const { data: categories } = useGetVendorCategoriesQuery();

  const categoryData = categories?.[0]?.categories || [];
  const allProducts = data?.beautyItems ?? [];
  ...
  if (isError) {
    return (
      <ErrorMessage
        title="Failed to load beauty products"
        message="We couldn’t fetch the beauty items right now. Please check your internet connection or try again."
        onRetry={refetch}
      />
    );
  }
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-semibold">
                {normalizeForDisplay(thirdcategory || subcategory || "Beauty")}
              </h1>
