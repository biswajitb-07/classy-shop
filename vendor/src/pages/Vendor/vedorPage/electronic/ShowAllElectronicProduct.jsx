// File guide: ShowAllElectronicProduct source file.
...
} from "../../../../features/api/electronic/electronicApi";
import {
  FaTrash,
  FaSync,
  FaEdit,
...
const ShowAllElectronicProduct = () => {
  ...
  const [deleteElectronicItem, { isLoading: isDeleting }] =
    useDeleteElectronicItemMutation();
  const [updateElectronicItem, { isLoading: isUpdating }] =
    useUpdateElectronicItemMutation();
  const [updateElectronicImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateElectronicImagesByIndexMutation();
  const { data: categoryData, refetch: categoryRefetch } =
    useGetVendorCategoriesQuery();
  const { data: electronicBrands, refetch: brandRefetch } =
  ...
  const handleMultiImageUpdate = async (electronicId) => {
    const updateMap = selectedUpdates[electronicId] || {};
    const additionalImages = newImages[electronicId] || [];
    ...
    try {
      await updateElectronicImagesByIndex({
        id: electronicId,
        body: formData,
      }).unwrap();
  ...
  const handleFieldChange = (field, value) => {
    ...
    } else if (field === "rams" || field === "storage") {
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      const payload = {
        ...editItem,
        ...editFields,
        rams: editFields.rams || editItem.rams || [],
        storage: editFields.storage || editItem.storage || [],
        rating: ratingValue,
      };
      await updateElectronicItem({ id: editItem._id, body: payload }).unwrap();
              ...
              <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-xs sm:text-sm mt-1 sm:mt-2 flex items-center gap-2">
                    ...
                    Manage your electronic inventory with style
                  </p>
                </div>
              </div>
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no electronic products in your inventory. Start by adding some amazing products!"
        ...
        <table className="w-full min-w-[1000px]">
          <thead>
            <tr className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
              ...
              <th className="py-3 px-4 text-left text-sm font-semibold">RAM</th>
              <th className="py-3 px-4 text-left text-sm font-semibold">
                Storage
              </th>
              <th className="py-3 px-4 text-left text-sm font-semibold">
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-100">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                      <FaShapes className="text-purple-500" /> Available RAMs
                    </label>
                      ...
                      {RAMS.map((r) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            value={r}
                            checked={editFields.rams?.includes(r) || false}
                            onChange={(e) =>
                              handleFieldChange("rams", { target: e.target })
                            }
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                      <FaShapes className="text-purple-500" /> Available
                      Storages
                    </label>
                    <div className="flex flex-wrap gap-4">
                      {STORAGES.map((s) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            value={s}
                            checked={editFields.storage?.includes(s) || false}
                            onChange={(e) =>
                              handleFieldChange("storage", { target: e.target })
                            }
                            className="h-5 w-5 text-indigo-600 rounded focus:ring-indigo-500"
                          />
