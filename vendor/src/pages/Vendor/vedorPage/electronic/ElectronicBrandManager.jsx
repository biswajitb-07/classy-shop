// File guide: ElectronicBrandManager source file.
...
} from "../../../../features/api/electronic/electronicBrandApi";
import {
  FaTrash,
  FaPlus,
  FaEdit,
...
const ElectronicBrandManager = () => {
  const [deletingBrand, setDeletingBrand] = useState(null);

  const { data, isLoading, isError } = useGetElectronicBrandsQuery();
  const [addElectronicBrand, { isLoading: isAdding }] =
    useAddElectronicBrandMutation();
  const [deleteElectronicBrand] = useDeleteElectronicBrandMutation();
  const [updateElectronicBrandList, { isLoading: isUpdating }] =
    useUpdateElectronicBrandListMutation();
  ...
  const handleAddBrand = async () => {
    ...
    try {
      await addElectronicBrand({ newBrand: newBrand.trim() }).unwrap();
      setNewBrand("");
  ...
  const handleDeleteBrand = async (brand) => {
    ...
    try {
      await deleteElectronicBrand({ brandToRemove: brand }).unwrap();
  ...
  const handleSaveEdit = async (oldBrand) => {
    ...
    try {
      await updateElectronicBrandList({ brand: updatedBrands }).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-gray-100 pb-5">
      <div className="container mx-auto">
        {/* Header Section */}
        <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-gray-600 rounded-3xl shadow-2xl mb-6 sm:mb-8">
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 sm:mb-2">
                  Electronic Brand Manager
                </h1>
                <p className="text-blue-100 text-sm sm:text-base md:text-lg">
                  Create and manage your premium electronics collection
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder="✨ Enter premium brand name..."
                    value={newBrand}
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 overflow-hidden">
          <div className="p-6 sm:p-8">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <div className="w-8 sm:w-10 h-8 sm:h-10 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl flex items-center justify-center">
                <FaStar className="text-white text-base sm:text-lg" />
              </div>
              <h2 className="text-base md:text-xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                Premium Brand Collection
              </h2>
              <div className="ml-auto text-center bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-bold">
            ...
            <div className="space-y-4 max-h-[24rem] sm:max-h-96 overflow-y-auto pr-2 custom-scrollbar">
              {isLoading ? (
                [...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="h-16 sm:h-20 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-2xl animate-pulse shadow-md"
                  ></div>
                ))
              ) : brands.length > 0 ? (
                brands.map((brand, index) => (
                  <div
                    key={index}
                    className="group relative bg-gradient-to-r from-white via-blue-50/30 to-indigo-50/30 backdrop-blur-sm border border-blue-100/50 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-500/0 via-indigo-500/0 to-gray-500/0 group-hover:from-blue-500/5 group-hover:via-indigo-500/5 group-hover:to-gray-500/5 transition-all duration-300"></div>
                      ...
                      {editingIndex === index ? (
                        ...
                        <>
                          <div className="flex items-center gap-3 sm:gap-4">
                            <div className="w-10 sm:w-12 h-10 sm:h-12 bg-gradient-to-r from-blue-400 to-indigo-400 rounded-xl flex items-center justify-center shadow-lg">
                ...
                <div className="text-center py-12 sm:py-16">
                  <div className="w-20 sm:w-24 h-20 sm:h-24 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  ...
                  <p className="text-gray-500 text-sm sm:text-lg">
                    Start building your premium brand collection above!
                  </p>
                </div>
              )}
            </div>
...
export default ElectronicBrandManager;
