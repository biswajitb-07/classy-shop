// File guide: AddElectronicItem source file.
// This file belongs to the vendor app architecture and has a focused responsibility within its module/folder.
import { useState } from "react";
import { useAddElectronicItemMutation } from "../../../../features/api/electronic/electronicApi";
import {
  FaPlus,
  FaTimes,
  FaImage,
...
const AddElectronicItem = ({
  ...
  const [addProduct, { isLoading }] = useAddElectronicItemMutation();
  ...
  const handleRam = (e) => {
    ...
    setForm((prev) =>
      checked
        ? { ...prev, rams: [...prev.rams, value] }
        : { ...prev, rams: prev.rams.filter((r) => r !== value) }
    );
  };
  ...
  const handleStorage = (e) => {
    ...
    setForm((prev) =>
      checked
        ? { ...prev, storage: [...prev.storage, value] }
        : { ...prev, storage: prev.storage.filter((s) => s !== value) }
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    form.rams.forEach((ram) => formData.append("rams", ram));
    form.storage.forEach((stor) => formData.append("storage", stor));
  ...
  const brands = electronicBrands.brand || [];
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-4 sm:p-6 rounded-xl border border-purple-100">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <div>
                    <label className="text-xs sm:text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                      <FaShapes className="text-purple-500" />
                      Available RAMs
                    </label>
                      ...
                      {STORAGES.map((s) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            value={s}
                            checked={form.storage.includes(s)}
...
export default AddElectronicItem;
