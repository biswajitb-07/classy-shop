// File guide: ShowAllJewelleryProduct source file.
...
} from "../../../../features/api/jewellery-product/jewelleryApi";
import {
  FaTrash,
  FaSync,
  FaEdit,
...
const ShowAllJewelleryProduct = () => {
  const {
    data: response,
    isLoading,
    refetch,
  } = useGetJewelleryItemsQuery(undefined, {
    refetchOnMountOrArgChange: false,
  });
  const [deleteJewelleryItem, { isLoading: isDeleting }] =
    useDeleteJewelleryItemMutation();
  const [updateJewelleryItem, { isLoading: isUpdating }] =
    useUpdateJewelleryItemMutation();
  const [updateJewelleryImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateJewelleryImagesByIndexMutation();
  ...
  const items = useMemo(() => response?.jewelleryItems ?? EMPTY_ITEMS, [response]);
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      await deleteJewelleryItem(id).unwrap();
  ...
  const handleMultiImageUpdate = async (jewelleryId) => {
    const updateMap = selectedUpdates[jewelleryId] || {};
    const additionalImages = newImages[jewelleryId] || [];
    ...
    try {
      await updateJewelleryImagesByIndex({
        id: jewelleryId,
        body: formData,
      }).unwrap();
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      await updateJewelleryItem({ id: editItem._id, body: payload }).unwrap();
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no jewellery products in your inventory. Start by adding some amazing products!"
...
export default ShowAllJewelleryProduct;
