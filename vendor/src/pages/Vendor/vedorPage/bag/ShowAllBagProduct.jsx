// File guide: ShowAllBagProduct source file.
...
const ShowAllBagProduct = () => {
  const {
    ...
    isLoading,
    refetch,
  } = useGetBagItemsQuery(undefined, {
    refetchOnMountOrArgChange: false,
  });
  const [deleteBagItem, { isLoading: isDeleting }] = useDeleteBagItemMutation();
  const [updateBagItem, { isLoading: isUpdating }] = useUpdateBagItemMutation();
  const [updateBagImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateBagImagesByIndexMutation();
  const { data: categoryData, refetch: categoryRefetch } =
    useGetVendorCategoriesQuery(undefined, {
      refetchOnMountOrArgChange: false,
    });
  const { data: bagBrands, refetch: brandRefetch } = useGetBagBrandsQuery(
    undefined,
    {
      refetchOnMountOrArgChange: false,
    },
  ...
  const handleMultiImageUpdate = async (bagId) => {
    const updateMap = selectedUpdates[bagId] || {};
    const additionalImages = newImages[bagId] || [];
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      await updateBagItem({ id: editItem._id, body: payload }).unwrap();
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no bag products in your inventory. Start by adding some amazing products!"
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-100">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                          <FaBoxOpen className="text-purple-500" /> Stock
                          Quantity
                        </label>
                        <input
                          type="number"
