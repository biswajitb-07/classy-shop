// File guide: BagBrandManager source file.
...
} from "../../../../features/api/bag/bagBrandApi";
import {
  FaTrash,
  FaPlus,
  FaEdit,
...
const BagBrandManager = () => {
  const [deletingBrand, setDeletingBrand] = useState(null);
  const [newBrand, setNewBrand] = useState("");
  const [editingIndex, setEditingIndex] = useState(null);
  const [editValue, setEditValue] = useState("");

  const { data, isLoading, isError } = useGetBagBrandsQuery();
  const [addBagBrand, { isLoading: isAdding }] = useAddBagBrandMutation();
  const [deleteBagBrand] = useDeleteBagBrandMutation();
  const [updateBagBrandList, { isLoading: isUpdating }] =
    useUpdateBagBrandListMutation();
  ...
  const handleAddBrand = async () => {
    ...
    try {
      await addBagBrand({ newBrand: newBrand.trim() }).unwrap();
      setNewBrand("");
      toast.success("Bag brand added!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to add bag brand.");
    }
  };
  ...
  const handleDeleteBrand = async (brand) => {
    ...
    try {
      await deleteBagBrand({ brandToRemove: brand }).unwrap();
      toast.error("Bag brand deleted!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to delete bag brand.");
    } finally {
  ...
  const handleSaveEdit = async (oldBrand) => {
    ...
    try {
      await updateBagBrandList({ brand: updatedBrands }).unwrap();
      toast.success("Bag brand updated!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to update bag brand.");
    } finally {
      handleCancelEdit();
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-red-100 flex items-center justify-center p-4">
        <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-2xl text-center max-w-md w-full">
          ...
          <p className="text-red-600 font-medium text-sm sm:text-base">
            Failed to load bag brands. Please try again.
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm sm:text-base md:text-lg">
                  Create and manage your luxury bag brand collection
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder="✨ Enter luxury bag brand name..."
                    value={newBrand}
              ...
              {isLoading ? (
                ...
                <div className="text-center py-12 sm:py-16">
                  ...
                  <p className="text-gray-500 text-sm sm:text-lg">
                    Start building your luxury bag brand collection above!
                  </p>
                </div>
              )}
            </div>
...
export default BagBrandManager;
