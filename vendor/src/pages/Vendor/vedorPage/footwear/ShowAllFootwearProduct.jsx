// File guide: ShowAllFootwearProduct source file.
...
} from "../../../../features/api/footwear-product/footwearApi";
import {
  FaTrash,
  FaSync,
  FaEdit,
...
const ShowAllFootwearProduct = () => {
  ...
  const [deleteFootwearItem, { isLoading: isDeleting }] =
    useDeleteFootwearItemMutation();
  const [updateFootwearItem, { isLoading: isUpdating }] =
    useUpdateFootwearItemMutation();
  const [updateFootwearImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateFootwearImagesByIndexMutation();
  const { data: categoryData, refetch: categoryRefetch } =
    useGetVendorCategoriesQuery();
  const { data: footwearBrands, refetch: brandRefetch } =
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      await deleteFootwearItem(id).unwrap();
  ...
  const handleMultiImageUpdate = async (footwearId) => {
    const updateMap = selectedUpdates[footwearId] || {};
    const additionalImages = newImages[footwearId] || [];
    ...
    try {
      await updateFootwearImagesByIndex({
        id: footwearId,
        body: formData,
      }).unwrap();
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      await updateFootwearItem({ id: editItem._id, body: payload }).unwrap();
              ...
              <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-xs sm:text-sm mt-1 sm:mt-2 flex items-center gap-2">
                    ...
                    Manage your footwear inventory with style
                  </p>
                </div>
              </div>
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no footwear products in your inventory. Start by adding some amazing products!"
