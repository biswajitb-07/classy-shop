// File guide: AddFootwearItem source file.
// This file belongs to the vendor app architecture and has a focused responsibility within its module/folder.
import { useState } from "react";
import { useAddFootwearItemMutation } from "../../../../features/api/footwear-product/footwearApi";
import {
  FaPlus,
  FaTimes,
  FaImage,
...
const AddFootwearItem = ({
  ...
  const [addProduct, { isLoading }] = useAddFootwearItemMutation();
  ...
  const handleSize = (e) => {
    ...
    setForm((prev) =>
      checked
        ? { ...prev, sizes: [...prev.sizes, value] }
        : { ...prev, sizes: prev.sizes.filter((s) => s !== value) }
    );
  };
  ...
  const brands = footwearBrands?.brand || [];
...
export default AddFootwearItem;
