...
} from "../../../../features/api/wellness-product/wellnessApi";
import {
  FaTrash,
  FaSync,
  FaEdit,
...
const ShowAllWellnessProduct = () => {
  const {
    ...
    isLoading,
    refetch,
  } = useGetWellnessItemsQuery(undefined, {
    refetchOnMountOrArgChange: false,
  });
  const [deleteWellnessItem, { isLoading: isDeleting }] =
    useDeleteWellnessItemMutation();
  const [updateWellnessItem, { isLoading: isUpdating }] =
    useUpdateWellnessItemMutation();
  const [updateWellnessImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateWellnessImagesByIndexMutation();
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      await deleteWellnessItem(id).unwrap();
  ...
  const handleMultiImageUpdate = async (wellnessId) => {
    const updateMap = selectedUpdates[wellnessId] || {};
    const additionalImages = newImages[wellnessId] || [];
    ...
    try {
      await updateWellnessImagesByIndex({
        id: wellnessId,
        body: formData,
      }).unwrap();
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      await updateWellnessItem({ id: editItem._id, body: payload }).unwrap();
              ...
              <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-xs sm:text-sm mt-1 sm:mt-2 flex items-center gap-2">
                    ...
                    Manage your wellness inventory
                  </p>
                </div>
              </div>
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no wellness products in your inventory. Start by adding some amazing products!"
