// File guide: wellnessBrandManager source file.
...
} from "../../../../features/api/wellness/welllnessBrandApi";
import {
  FaTrash,
  FaPlus,
  FaEdit,
...
const WellnessBrandManager = () => {
  ...
  const { data, isLoading, isError } = useGetWellnessBrandsQuery();
  const [addWellnessBrand, { isLoading: isAdding , error}] =
    useAddWellnessBrandMutation();
  const [deleteWellnessBrand] = useDeleteWellnessBrandMutation();
  const [updateWellnessBrandList, { isLoading: isUpdating }] =
    useUpdateWellnessBrandListMutation();
  ...
  const handleAddBrand = async () => {
    if (!newBrand.trim())
      return toast.error("Wellness brand name is required.");
    try {
      await addWellnessBrand({ newBrand: newBrand.trim() }).unwrap();
      setNewBrand("");
      toast.success("Wellness brand added!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to add wellness brand.");
    }
  };
  ...
  const handleDeleteBrand = async (brand) => {
    ...
    try {
      await deleteWellnessBrand({ brandToRemove: brand }).unwrap();
      toast.error("Wellness brand deleted!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to delete wellness brand.");
    } finally {
  ...
  const handleSaveEdit = async (oldBrand) => {
    if (!editValue.trim())
      return toast.error("Wellness brand name can't be empty.");
    const updatedBrands = brands.map((b) =>
      b === oldBrand ? editValue.trim() : b
    );
    try {
      await updateWellnessBrandList({ brand: updatedBrands }).unwrap();
      toast.success("Wellness brand updated!");
    } catch (err) {
      toast.error(err?.data?.message || "Failed to update wellness brand.");
    } finally {
      handleCancelEdit();
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-red-100 flex items-center justify-center p-4">
        <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-2xl text-center max-w-md w-full">
          ...
          <p className="text-red-600 font-medium text-sm sm:text-base">
            Failed to load wellness brands. Please try again.
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 sm:mb-2">
                  Wellness Brand Manager
                </h1>
                <p className="text-blue-100 text-sm sm:text-base md:text-lg">
                  Create and manage your wellness brand collection
                </p>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder="✨ Enter wellness brand name..."
                    value={newBrand}
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              ...
              <h2 className="text-base md:text-xl font-bold bg-gradient-to-r from-blue-600 to-teal-600 bg-clip-text text-transparent">
                Premium Wellness Brand Collection
              </h2>
              ...
              {isLoading ? (
                ...
                <div className="text-center py-12 sm:py-16">
                  ...
                  <h3 className="text-lg sm:text-xl font-bold text-gray-600 mb-2">
                    No wellness brands yet
                  </h3>
                  <p className="text-gray-500 text-sm sm:text-lg">
                    Start building your wellness brand collection above!
                  </p>
                </div>
              )}
            </div>
