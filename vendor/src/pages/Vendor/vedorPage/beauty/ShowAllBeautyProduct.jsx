// File guide: ShowAllBeautyProduct source file.
...
} from "../../../../features/api/beauty-product/beautyApi";
import {
  FaTrash,
  FaSync,
  FaEdit,
...
const ShowAllBeautyProduct = () => {
  const {
    data: response,
    isLoading,
    refetch,
  } = useGetBeautyItemsQuery(undefined, {
    refetchOnMountOrArgChange: false,
  });
  const [deleteBeautyItem, { isLoading: isDeleting }] =
    useDeleteBeautyItemMutation();
  const [updateBeautyItem, { isLoading: isUpdating }] =
    useUpdateBeautyItemMutation();
  const [updateBeautyImagesByIndex, { isLoading: isUpdatingImages }] =
    useUpdateBeautyImagesByIndexMutation();
  ...
  const items = useMemo(() => response?.beautyItems ?? EMPTY_ITEMS, [response]);
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      await deleteBeautyItem(id).unwrap();
  ...
  const handleMultiImageUpdate = async (beautyId) => {
    const updateMap = selectedUpdates[beautyId] || {};
    const additionalImages = newImages[beautyId] || [];
    ...
    try {
      await updateBeautyImagesByIndex({
        id: beautyId,
        body: formData,
      }).unwrap();
  ...
  const handleUpdateItemFields = async () => {
    ...
    try {
      ...
      await updateBeautyItem({ id: editItem._id, body: payload }).unwrap();
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center py-12 sm:py-16 bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20">
      ...
      <p className="text-gray-600 mb-6 sm:mb-8 max-w-md text-center text-sm sm:text-base">
        ...
        selectedSubCategory === "all" &&
        selectedThirdLevel === "all"
          ? "There are currently no beauty products in your inventory. Start by adding some amazing products!"
...
export default ShowAllBeautyProduct;
