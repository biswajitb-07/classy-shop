...
export const getBrandsByVendor = async (req, res) => {
  try {
    ...
    const brandDoc = await ElectronicBrand.findOne({ vendorId });
    if (!brandDoc) {
...
export const addElectronicBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    let brandDoc = await ElectronicBrand.findOne({ vendorId });

    if (!brandDoc) {
      brandDoc = await ElectronicBrand.create({
        vendorId,
        brand: [newBrand.trim()],
      });
    } else {
      brandDoc = await ElectronicBrand.findOneAndUpdate(
        { vendorId },
...
export const updateElectronicBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    const updatedBrandDoc = await ElectronicBrand.findOneAndUpdate(
      { vendorId },
...
export const deleteElectronicBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    const brandDoc = await ElectronicBrand.findOneAndUpdate(
      { vendorId },
