...
export const getBrandsByVendor = async (req, res) => {
  try {
    ...
    const brandDoc = await BagBrand.findOne({ vendorId });
    if (!brandDoc) {
...
export const addBagBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    let brandDoc = await BagBrand.findOne({ vendorId });

    if (!brandDoc) {
      brandDoc = await BagBrand.create({
        vendorId,
        brand: [newBrand.trim()],
      });
    } else {
      brandDoc = await BagBrand.findOneAndUpdate(
        { vendorId },
...
export const updateBagBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    const updatedBrandDoc = await BagBrand.findOneAndUpdate(
      { vendorId },
...
export const deleteBagBrand = async (req, res) => {
  try {
    const vendorId = req.id;
    ...
    const brandDoc = await BagBrand.findOneAndUpdate(
      { vendorId },
