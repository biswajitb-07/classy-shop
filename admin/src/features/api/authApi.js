...
export const authApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    toggleAdminBlock: builder.mutation({
      query: ({ id, isBlocked }) => ({
        url: `admins/${id}/block`,
        method: "PATCH",
        body: { isBlocked },
    ...
    getDeliveryPayoutDesk: builder.query({
      query: () => ({
        url: "delivery-payouts",
        method: "GET",
      }),
      providesTags: [{ type: "DeliveryPartners", id: "PAYOUT_DESK" }],
    }),
    approveDeliveryPayout: builder.mutation({
      query: (body) => ({
        url: "delivery-payouts/approve",
        method: "POST",
        body,
      }),
      invalidatesTags: [
        { type: "DeliveryPartners", id: "LIST" },
        { type: "DeliveryPartners", id: "PAYOUT_DESK" },
      ],
    }),
    updateDeliveryPayoutStatus: builder.mutation({
      query: ({ payoutId, status, processedNotes }) => ({
        url: `delivery-payouts/${payoutId}/status`,
        method: "PATCH",
        body: { status, processedNotes },
      }),
      invalidatesTags: [
